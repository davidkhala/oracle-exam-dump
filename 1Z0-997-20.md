...
1. You are creating an Oracle Cloud Infrastructure **Dynamic Group**. To determine the members of this group you are defining a set of **matching rules**. Which of the following are the supported variables to define conditions in the matching rules? (Choose Two) 
    ...
    - [x] instance.compartment.id - the OCID of the compartment where the instance reside in; 
    - [ ] iam.policy.ld - the OCID of the IAM polity to apply to the group. 
...
1. Multiple departments In your company use a shared Oracle Cloud Infrastructure (OCI) tenancy to Implement their projects. You are in charge of managing the cost of OCI resources in the tenancy and need to obtain better Insights Into department's usage. Which three options can you implement together to accomplish this? 
    ...
    - [ ] Set up a consolidated budget-tracking tags to analyze costs in a granular manner.
...
## Compartment and Tag
1. 14 applications node from different departments, each in its own Compartment, operation usergroup already have read all resource in tenancy, what’s is the most efficient way to set the group to manage only the 14 compartments without ability to modify other resources in the tenancy 
    - [ ] set policy to 14 compartments
    - [ ] use tagging on the instance and set policy in root compartment `allow opsgroup to manage all resource in tenancy where ... `
...
Which option is the simplest way to implement this new requirement? (Choose the best answer.) 
    ...
    - [ ] Create tag variables to automatically tag a resource with the user name. (Debate: this should be part of solution) 
    - [x] Create a default tag for each compartment, which ensure that appropriate tags are applied at resource creation 
...
1. Create compute instance using oracle image, which of the following is not possible
    - [ ] linux image cannot modify metadata before creation
    - [x] windows
...
1. You Two Machines as below: 
    ...
    - [x] B is having higher IOPS but lower throughput performance in comparison to A
        - [IOPS and KPBS performance scales linearly per GB volume size up to the service maximums](https://docs.oracle.com/en-us/iaas/Content/Block/Concepts/blockvolumeperformance.htm#Higher_Performance)
1. A cloud consultant is working on implementation project on OCI. As part of the compliance requirements. the objects placed in object storage should be automatically archived first and then deleted. He is testing a Lifecycle Policy on Object Storage and created a policy as below: 
    ```
    [( "name": "Archive doc", "action": "ARCHIVE", "objectNameFilter" :{ "inclusionPrefixes": "doc"}, "timeAmount": 5, "timeunit": "DAYS", "isEnabled": true}, ("name": "Delete_doc", "action": "DELETE". "objectNameFilter":{ "inclusionPrefixes": "doc"},"timeAmount": 5, "timeunit": "DAYS", "isEnabled": true ) 
    ```
...
1. Which three types of credentials are used to manage Oracle Cloud Infrastructure Identity and Access Management (IAM)? 
    ...
    - [x] API Signing Key
    - [x] Swift Password
        - suspicious old question: swift password is replaced by Auth Tokens now
    - [ ] SSH Key
    - [x] Console Password 
...
1. Compute instance using OS Management service (for downloading patch), installed agent but could not connect, what’s NOT the possible cause.
    ...
    - [x] Compute with private endpoint
...
1. You have designed and deployed your Autonomous Data Warehouse (ADW) such that it is accessible from your on-premises data center and servers running on both private and public networks in Oracle Cloud Infrastructure (OCI). As you are testing the connectivity to your ADW database from the different access paths, you notice that the server on the private network is unable to connect to ADW. 
    [image missing]
    Which two steps do you need to take to enable connectivity from the server on the private network to ADW? 
    - [ ] Add an entry in the Security List of the ADW allowing ingress traffic for CIOR block 10.2.2.0/24 
    - [x] Add an entry in the route table (associated with the private subnet) with destination of 0.0.0.0: target type of NAT Gateway, add a stateful egress rule to the security list (associated With the private subnet) with destination of 0.0.0.0./0 and for all IP protocols 
    - [ ] Add an entry in the access table list of ADW for CIDR block 10.2.2.0/24. 
    - [ ] Add an entry in the route table (associated with the private subnet) With destination of 0.0.0.0: target type of Internet Gateway, add a stateful egress in the security list (associated with the private subnet) With destination of 0.0.0.0/0 and for all IP protocols. 
...
1. Which two are required parameters to create a public load balancer instance? 
    ...
    - [x] two public subnets
        - A public load balancer requires either a regional subnet or two AD-specific subnets
    - [ ] certificate 
...
1. You have deployed a web application targeting a global audience across multiple Oracle Cloud Infrastructure (OCI) regions. You decide to use Traffic Management Geolocation based Steering Policy to serve web requests to users from the **region closets to the user**. Within each region you have deployed a public load balancer with 4 servers in a backend set. During a DR test disable all web servers in one of the regions however, traffic Management does not automatically direct all users to the other region. Which two are possible causes? 
    ...
    - ?? Why not BC? OCI LB is Regional service and cannot configured as cross regional 
    - Glenn: geolocation also have failover capability
...
## Oracle K8S Engine
1. K8S, multiple endpoint, cost effective to expose endpoint without introducing architecture complexity:
    - [ ] Multiple Load Balancer each 100mb 
    - [ ] Ingress Controller
1. K8S: SSL termination, 
    - [ ] create self-signed cert, install to load balancer and all “service” using the load balancer
    - 3 answers install cert and key to OCIR, use annotation in YAML? 
...
1.  A cost conscious fashion design company which sells bags, clothes, and other luxury items has recently decided to move all of the their on-premises infrastructure to Oracle Cloud Infrastructure (OCI), One of their on-premises application is running on an NGINX server and the Oracle Database is running in a 2 node Oracle Real Application Clusters (RAC) configuration. Based on cost considerations, what is an effective mechanism to migrate the customer application to OCI and set up regular automated backups? 
    ...
    - [x] Launch a compute Instance and run an NGINX server to host the application. Deploy a 2 node VM DB Systems with Oracle RAC enabled. Import the on-premises database to OCI VM DB Systems using Data Pump and then enable automatic backups.
...
Which of the following needs to be considered while configuring auto scaling? Choose two 
    ...
    - [ ] The database memory SGA and PGA will not get effected by the changes in the number of CPUs during auto scaling 
    - [x] Enabling auto scaling does not change the concurrency and parallelism settings 
    - [x] Auto scaling also scales I/O throughput linearly along With CPU 
1. You have just created an Autonomous Data Warehouse (ADW) and you want to connect to the ADW using SQL Developer. What three items are needed to connect to the ADW using SQL Developer? 
    - [ ] the public IP address of the ADW server
    - [x] the key store password
    - [x] the client credentials file
    - [x] the admin password 
    - [ ] a security list with an ingress rule for TCP port 1521 
    - the client credential file have already DB connection material, so no need of additional DB address info.
        - https://technology.amis.nl/cloud/connect-local-sql-developer-to-oracle-cloud-autonomous-database-always-free-tier/
...
Which two options do you need to implement together to migrate your on-premises databases to OCI? (Choose two.)
    ...
    - [x] Convert on-premises databases to PDB, upgrade to 19c, and encrypt.
    - [ ] Retain all legacy structures and unsupported features (e.g. legacy LOBs) in the on-premises databases for migration.
1. Which three scenarios are suitable for the Oracle Infrastructure (OCI) Autonomous Transaction Processing Shared (ATP-S) deployment? 
